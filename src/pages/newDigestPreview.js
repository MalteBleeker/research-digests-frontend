...
function NewDigestPreview(props) {
  ...
  useEffect(() => {
    ...
    const handleAuth = async () => {
      if (code) {
        try {
          ...
          const functionRef = httpsCallable(functions, 'guestAccess');
          const response = await functionRef({ code: code?.toUpperCase(), id: id });
    ...
    // if no auth user, show password modal
    useEffect(() => {
            ...
            <div style={{...styles.mainContainerMobile, border: "none", boxShadow: borderColor, marginLeft: 0, minWidth: "102%", padding: 0}}>
              ...
              <div style={{display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-start",margin: '0 auto', marginTop: "10px"}}>
                ...
                <div style={{width: "82%", marginBottom: 0}}>
                  <div style={{fontSize: 15, color: "grey"}}>{formatAuthors(article?.cAuthors)}</div>
                </div>
                <div style={{}}>
                <p
                  ...
                  dangerouslySetInnerHTML={{
                    __html: `${article?.cContent}`, // Append the black square
                  }}
                ></p>
                </div>
                        ...
                        <div style={{marginTop: 0}}>
                          <p
                            ...
                            style={{
                              ...
                              textAlign: 'left',
                              margin: '0 auto',
                          }} dangerouslySetInnerHTML={{ __html: article?.cMethodText }}></p>
                        </div>
                      }
                      {article?.cReference && (
                        <p style={{ fontSize: 15, lineHeight: '25px', }} >
                          Source: {article?.cReference}
                          &nbsp;
                          <a href={article?.cSourceUrl ? formatUrl(article?.cSourceUrl) : null} style={{ fontSize: 15, color: "grey" }} target="_blank" rel="noopener noreferrer">
                            {article?.cSourceUrl}
                          </a>
                        </p>
                      )}
        ...
        <>
          <div style={{...styles.startContainer, marginLeft: XL  || L ? "15vw" : M ? "16vw" : "18vw", marginRight: XL ? "1vw" : "2vw"}}>
            {loading ? (<></>) : (
              ...
              {passwordModal && (
                <div style={{position: "fixed", top: 0, left: 0, width: "100vw", height: "100vh", backgroundColor: "rgba(0,0,0,0.5)", zIndex: 10, display: "flex", justifyContent: "center", alignItems: "center"}}>
                  <div style={{backgroundColor: "white", width: "500px", padding: 20, borderRadius: 10, marginBottom: "40vh", boxShadow: "0 4px 8px 0 rgba(128,128,128,0.4)"}}>
                    <div style={{fontSize: 30, fontWeight: "bold"}}>Welcome to Research Digests!</div>
                    <p>Thank you for sharing knowledge! Please enter the code you received in the invite to view, edit or publish this digest. </p>
                    <input 
                    placeholder='Code...' 
                    value={code}
                    onChange={(e) => setCode(e.target.value)}
                    style={{ width: '120px', padding: '5px', marginBottom: '10px' }} />
                    <div style={{display: "flex", justifyContent: "flex-start"}}>
                      <div
                        onClick={() => handleAuth()}
                        style={{padding: 8, width: 70, cursor: "pointer", backgroundColor: "black", color: "white", marginTop: 10, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center"}}>
                        Submit
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", margin: '0 auto',}}>
                    <div style={{flexDirection: "row", display: "flex", justifyContent: "space-between", width: '89%', fontSize: 23, marginBottom: 20}}>
                      ...
                      <button className="voteButton">Downvote</button>
                        ...
                        <h1 className="overflow" style={{fontSize: 30 }}>{article?.cTitle}</h1>
                        <p style={{fontSize: 23}}>{formatAuthors(article?.cAuthors)}</p>
                      </div>
                    <div style={{flexDirection: "column", display: "flex", justifyContent: "flex-start", alignItems: "center"}}>
                      <img 
                        src={article?.cCoverUrl} 
                    ...
                    style={{
                      ...
                      }} dangerouslySetInnerHTML={{ __html: article?.cContent }}></p>
                    </div>
                    <div style={{marginTop: 30, justifyContent: "flex-start", display: "flex", width:'89%', fontSize: 23, flexDirection: "column"}}>
                      ...
                      {article?.cReference && (
                        <p style={{ fontSize: 20 }}>
                          Source: {article?.cReference}
                          &nbsp;
                          <a href={article?.cSourceUrl ? formatUrl(article?.cSourceUrl) : null} style={{ fontSize: 16, color: "grey" }} target="_blank" rel="noopener noreferrer">
                            {article?.cSourceUrl}
                          </a>
                        </p>
                      )}
...
export default NewDigestPreview;